...
it('TEST-002/009: concurrent staging reserves shared bytes until cancellation cleanup; no eviction', async t => {
  ...
  const staged = latch(), controller = new AbortController();
  async function* pending() {yield new Uint8Array(60); staged.release(); await new Promise<void>(() => {});}
  ...
  controller.abort(); await put; await f.adapter.close(); assert.deepEqual(await readdir(f.root), []); assert.equal(f.requests.filter(r => r.method === 'PUT').length, 0);
});
