...
| --- | --- | --- | --- |
| AC-001 | 所有 key/prefix 阴性拒绝且无 I/O 副作用，合法值保真 | TEST-001 | Memory PASS / Local PASS（测试证据） |
| AC-002 | 默认写竞争只发布一个完整对象，失败保留旧值 | TEST-002 | Memory PASS / Local PASS（测试证据） |
| AC-003 | 读 snapshot 一致，exists 不吞权限/网络错误 | TEST-003 | Memory PASS / Local PASS（测试证据） |
| AC-004 | 单 key delete/ifRevision 并发语义正确 | TEST-004 | Memory PASS / Local PASS（测试证据） |
| AC-005 | 有界分页、续页和 namespace 隔离正确 | TEST-005 | Memory PASS / Local PASS（测试证据） |
| AC-006 | copy 保留数据与 metadata，目标条件不被绕过 | TEST-006 | Memory PASS / Local PASS（测试证据） |
| AC-007 | 未支持 move 明确无副作用失败；无虚假原子承诺 | TEST-007 | Memory PASS / Local PASS（测试证据） |
| AC-008 | 只报告可兑现能力、Range 和条件语义不降级 | TEST-008 | Memory PASS / Local PASS（测试证据） |
| AC-009 | Memory/Local 不生成假签名；multipart 不启用 | TEST-009 | Memory PASS / Local PASS（测试证据） |
