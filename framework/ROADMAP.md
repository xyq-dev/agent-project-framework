...
下文 M0～M4 保留为原 Framework/Runtime 演进路线。M0 已完成；Storage M1-A 规格已完成；用户已恢复 Core/Memory/Local/OSS，当前四层初始 profile 已实现并有 112 项测试，原始 ST-007 Memory/Local 范围已获独立正式验收，OSS 离线范围通过审查；真实 OSS 与全模块验收待完成。新项目采用标准不以 Runtime 路线完成为前提。
...
- M1-B：Node/TypeScript Core + Memory 已实现，112 项合计测试通过，见[实施报告](../modules/storage/STORAGE_M1_B_IMPLEMENTATION_REPORT.md)。Local/OSS 设计已获独立通过，代码和本地测试已完成；原始 ST-007 Memory/Local 范围已获[独立正式验收](../modules/storage/STORAGE_M1_ACCEPTANCE_REVIEW.md)，后续执行[真实 OSS 验收清单](../modules/storage/OSS_CLOUD_VALIDATION.md)；含 OSS 的全模块仍 TESTING。
